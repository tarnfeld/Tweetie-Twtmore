...
+ (BOOL)jr_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
	Method origMethod = class_getClassMethod([self class], origSel_);
    Method newMethod = class_getClassMethod([self class], altSel_);
	
    Class c = object_getClass((id)[self class]);
	
    if(class_addMethod(c, origSel_, method_getImplementation(newMethod), method_getTypeEncoding(newMethod)))
        class_replaceMethod(c, altSel_, method_getImplementation(origMethod), method_getTypeEncoding(origMethod));
    else
